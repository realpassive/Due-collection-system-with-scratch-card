...
function insert_tracker($email){
   $ip = getenv('REMOTE_ADDR');
    $login_time = strtotime('now');
    $date = date("Y-d-m H:i:s");
   $sql = "UPDATE nass_login SET ip = '$ip',lastlogin = '$date',spent_time = '$login_time' WHERE email = '$email'";
           $query = query($sql);
    
     
}
function nass_check_role($email){
    $sql = "SELECT role_id FROM nass_login WHERE email='$email'";
         $query = query($sql);
         $count = mysqli_num_rows($query);
         $result = mysqli_fetch_array($query);
             $user_role = $result[0];
      ...
      '<div style="color:red;-align:center;">'.die('<strong>'.'you do not have access to this page because you role is not yet assign'.'</strong>').'</div>';
         }
     elseif($user_role != '2' || $user_role != 3 || $user_role != 4 || $user_role != 5){
         
           '<div class=\'lock\'>'.die('you do not have access to this page').'</div>';
